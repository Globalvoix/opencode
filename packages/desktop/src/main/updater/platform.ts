    ...
    checkForUpdate: Effect.tryPromise({
      try: async () => {
        if (external) {
          ...
          if (!response.ok) throw new Error(`Stable Thinksoft update check failed: ${response.status}`)
          const download = stableMacDownload(await response.json(), process.arch)
          if (!download) throw new Error("Stable Thinksoft download is unavailable")
