...
test("selects the signed stable installer for the current Mac architecture", () => {
  const artifact = {
    ...
    metadata: {
      files: {
        "thinksoft-desktop-mac-arm64.dmg": { url: "https://files.test/Thinksoft-arm64.dmg" },
        "thinksoft-desktop-mac-x64.dmg": { url: "https://files.test/Thinksoft-x64.dmg" },
      },
    },
  }
  ...
  expect(stableMacDownload(artifact, "arm64")).toEqual({
    version: "2.0.2",
    url: "https://files.test/Thinksoft-arm64.dmg",
  })
  expect(stableMacDownload(artifact, "x64")).toEqual({
    version: "2.0.2",
    url: "https://files.test/Thinksoft-x64.dmg",
  })
...
test("still resolves the pre-rebrand installer name", () => {
  const artifact = {
    version: "2.0.1",
    metadata: {
      files: {
