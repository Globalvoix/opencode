...
for (const channel of channels) {
  ...
  test(`ships Thinksoft-branded installers for ${channel.channel}`, async () => {
    const previous = process.env.OPENCODE_CHANNEL
    process.env.OPENCODE_CHANNEL = channel.channel
    const config = (await import(`./electron-builder.config.ts?brand=${channel.channel}`)).default as Configuration
    if (previous === undefined) delete process.env.OPENCODE_CHANNEL
    else process.env.OPENCODE_CHANNEL = previous

    // The installer dialog title, the installed executable and the "app is running"
    // check all derive from these, so an OpenCode value here is what makes setup
    // show "OpenCode Setup" and refuse to install over a running OpenCode.
    expect(config.productName).toStartWith("Thinksoft")
    expect(config.artifactName).toBe("thinksoft-desktop-${os}-${arch}.${ext}")
  })
