...
// etc.).
//
// The visual language mirrors the Thinksoft app: the design tokens are a curated
// subset of the semantic tokens in `packages/ui/src/styles/theme.css`, and the
// mark is the same geometry as `packages/ui/src/components/logo.tsx` (Splash).
...
export function success(options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider ? `Thinksoft is now connected to ${escapeHtml(provider)}.` : "Thinksoft is now authorized.",
...
export function error(detail: string, options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider
        ? `Thinksoft couldn't finish connecting to ${escapeHtml(provider)}.`
        : "Thinksoft couldn't complete authorization.",
      detail,
      footnote: "Close this window and try again from Thinksoft.",
    }),
  })
}
...
<html lang="en">
  <head>
    ...
    <title>${escapeHtml(input.title)} · Thinksoft</title>
    <style>${STYLES}</style>
  </head>
  <body>
...
(function(){
  ...
  function fail(text){card.dataset.status="error";headline.textContent="Authorization failed";message.textContent=PROVIDER?("Thinksoft couldn't finish connecting to "+PROVIDER+"."):"Thinksoft couldn't complete authorization.";if(text){detail.textContent=text;detail.hidden=false}footnote.textContent="Close this window and try again from Thinksoft."}
  function ok(){card.dataset.status="success";headline.textContent="Authorization successful";message.textContent=PROVIDER?("Thinksoft is now connected to "+PROVIDER+"."):"Thinksoft is now authorized.";detail.hidden=true;footnote.textContent="You can close this window.";setTimeout(function(){try{window.close()}catch(e){}},2500)}
  try{
...
// Thinksoft mark — same geometry as packages/ui/src/components/logo.tsx (Splash), with a
// transparent interior so it reads on both the light and dark callback backgrounds.
const WORDMARK = `<svg class="wordmark" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 80 100" fill="none" aria-label="Thinksoft" role="img">
        <rect x="5" y="15" width="70" height="70" rx="16" fill="var(--oc-icon-strong)" />
        <rect x="8" y="18" width="64" height="64" rx="13" fill="var(--oc-bg)" />
        <polyline points="31.6,37.4 46.9,50 31.6,62.6" fill="none" stroke="var(--oc-icon-strong)" stroke-width="7.8" stroke-linecap="round" stroke-linejoin="round" />
        <rect x="49.9" y="54.8" width="16.8" height="7.8" rx="3.9" fill="var(--oc-icon-strong)" />
      </svg>`
