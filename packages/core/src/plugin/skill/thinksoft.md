...
Use this guide as the starting point for work involving Thinksoft itself. It
covers the core concepts needed to configure and customize Thinksoft, extend it
with plugins, and build integrations with the Thinksoft SDK, clients, and API.
...
Always answer for Thinksoft V2 unless the user explicitly asks about V1,
legacy Thinksoft, or migrating from V1.
...
CLI and TUI preferences are separate from Thinksoft's server and project
...
## [Thinksoft configuration](https://opencode.ai/v2/docs/config)

Thinksoft's server and project configuration uses JSON or JSONC. Include the
...
During ordinary project discovery, Thinksoft searches the current Location
...
OAuth credentials are stored outside the Thinksoft configuration. Do not ask for or
...
For any request to migrate Thinksoft configuration, agents, commands, skills,
...
Thinksoft uses a client-server architecture. Interfaces such as the TUI connect
to a background Thinksoft service, which owns sessions, configuration, plugins,
permissions, and tool execution.

Thinksoft normally discovers or starts the shared background service
...
Thinksoft exposes an HTTP API from its server. The API is described by an
OpenAPI document available from the running server at `/openapi.json`.

Use Thinksoft's built-in `api` command for local requests. It uses the same
...
Request bodies default to `Content-Type: application/json`. When Thinksoft is
...
For questions about connecting an application to Thinksoft over the network,
fetch the full [client guide](https://opencode.ai/v2/docs/build/client) before
answering.

`@opencode/client` is the generated TypeScript client for the Thinksoft HTTP
...
exposes typed Effects, Streams, and decoded Thinksoft schema values. Its
...
For questions about embedding Thinksoft directly in an application, fetch the
full [SDK guide](https://opencode.ai/v2/docs/build/sdk) before answering. The SDK
hosts Thinksoft in the application without opening an HTTP listener.
...
Thinksoft runs a client and a background server. Start by determining whether a
