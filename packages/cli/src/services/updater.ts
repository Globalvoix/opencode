...
const make = Effect.gen(function* () {
  ...
  const release = Effect.fnUntraced(function* (method?: Method) {
    ...
    const response = yield* Effect.tryPromise({
      try: (signal) =>
        ...
        new UpgradeError(
          {
            title: "Could not check for Thinksoft updates",
    ...
    const data: { version: string; metadata?: { package?: string } } = yield* Effect.tryPromise({
      try: () => response.json(),
        ...
        new UpgradeError(
          {
            title: "Could not read the Thinksoft update information",
            detail: errorDetail(cause),
            retry: "Try again in a few minutes.",
          },
          { cause },
  ...
  const runUpgrade = (input: {
    ...
    const failure = (detail: string, cause?: unknown) =>
      new UpgradeError(
        {
          title: input.title ?? `${installNames[input.method]} could not install Thinksoft`,
          detail,
    ...
    yield* Effect.scoped(
      Effect.gen(function* () {
        ...
        if (method === "curl") {
          ...
          yield* runUpgrade({
            ...
            title: "Could not download the Thinksoft installer",
            retry: "Check your network, then run opencode upgrade again.",
          })
          return yield* retaining(
            method,
            runUpgrade({
              ...
              title: "The Thinksoft installer failed",
            }),
          )
        }
      ...
      Effect.mapError((cause) =>
        cause instanceof UpgradeError
          ...
          : new UpgradeError(
              {
                title: "Could not prepare the Thinksoft upgrade",
                detail: errorDetail(cause),
  ...
  const inspect = Effect.fnUntraced(function* () {
    ...
    yield* Effect.logInfo("Thinksoft update available", { current, latest: version, action: next })
  ...
  const install = Effect.fnUntraced(function* (version: string) {
    ...
    yield* Effect.logInfo("updated Thinksoft", { from: current, to: version, method: detected })
    return true
  })
  ...
  const check = Effect.fn("cli.updater.check")(function* () {
    if (OPENCODE_LOCAL)
      return {
        ...
        message: "This build runs from a source checkout. Use an installed Thinksoft release to check for updates.",
      }
    const version = yield* latest()
